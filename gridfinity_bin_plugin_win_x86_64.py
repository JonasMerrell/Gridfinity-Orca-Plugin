    ...
    <section id="plateOpts" hidden>
      ...
      <div id="plateMmOpts" hidden>
        ...
        <div id="plateBufferAlignOpts" hidden style="margin-top:6px; margin-bottom:8px">
          <div style="font-size:11.5px; color:var(--muted); margin-bottom:3px">Width Buffer (X):</div>
          <div class="modes submodes" style="margin-bottom:6px">
            <label id="lbl_buf_x_left"><input type="radio" name="buf_x_align" id="buf_x_left" value="left"> Left</label>
            <label id="lbl_buf_x_center" class="active"><input type="radio" name="buf_x_align" id="buf_x_center" value="center" checked> Centered</label>
            <label id="lbl_buf_x_right"><input type="radio" name="buf_x_align" id="buf_x_right" value="right"> Right</label>
          </div>
          <div style="font-size:11.5px; color:var(--muted); margin-bottom:3px">Depth Buffer (Y):</div>
          <div class="modes submodes">
            <label id="lbl_buf_y_bottom"><input type="radio" name="buf_y_align" id="buf_y_bottom" value="bottom"> Bottom</label>
            <label id="lbl_buf_y_center" class="active"><input type="radio" name="buf_y_align" id="buf_y_center" value="center" checked> Centered</label>
            <label id="lbl_buf_y_top"><input type="radio" name="buf_y_align" id="buf_y_top" value="top"> Top</label>
          </div>
        </div>
      </div>
    ...
    for (var k = 0; k < m; k++) {
      ...
      if (relax === 0) {
        for (var j = 0; j < m; j++) {
          ...
          if (inTriangle(q, a, b, c)) { ok = false; break; }
        }
      }
      if (!ok) continue;
...
function derivePlate(p) {
  ...
  if (p.plateExact && p.plate_size_mode === "mm") {
    ...
    var alignX = p.buf_x_align || "center";
    if (alignX === "left") {
      padLeft = remX; padRight = 0;
    } else if (alignX === "right") {
      padLeft = 0; padRight = remX;
    } else {
      padLeft = Math.round((remX / 2) * 100) / 100;
      padRight = Math.round((remX - padLeft) * 100) / 100;
    }

    var alignY = p.buf_y_align || "center";
    if (alignY === "bottom") {
      padBottom = remY; padTop = 0;
    } else if (alignY === "top") {
      padBottom = 0; padTop = remY;
    } else {
      padBottom = Math.round((remY / 2) * 100) / 100;
...
/* =====================================================================
   Renderer
   ===================================================================== */
var PREVIEW_SEG = { corner: 10, comp: 6, hole: 14 };
var RENDER_SEG  = { corner: 44, comp: 18, hole: 48 };
...
function readControls() {
  ...
  if (!isPlate) {
    ...
    var xAlign = "center";
    if (document.getElementById("buf_x_left") && document.getElementById("buf_x_left").checked) xAlign = "left";
    else if (document.getElementById("buf_x_right") && document.getElementById("buf_x_right").checked) xAlign = "right";
    P.buf_x_align = xAlign;

    var yAlign = "center";
    if (document.getElementById("buf_y_bottom") && document.getElementById("buf_y_bottom").checked) yAlign = "bottom";
    else if (document.getElementById("buf_y_top") && document.getElementById("buf_y_top").checked) yAlign = "top";
    P.buf_y_align = yAlign;

    ["buf_x_left", "buf_x_center", "buf_x_right"].forEach(function (id) {
      var lbl = document.getElementById("lbl_" + id);
      var el = document.getElementById(id);
      if (lbl && el) lbl.className = el.checked ? "active" : "";
    });
    ["buf_y_bottom", "buf_y_center", "buf_y_top"].forEach(function (id) {
      var lbl = document.getElementById("lbl_" + id);
      var el = document.getElementById(id);
      if (lbl && el) lbl.className = el.checked ? "active" : "";
    });
    ...
    if (fitInfo) {
      if (P.plateExact && isMm && (dp.padLeft > 0 || dp.padRight > 0 || dp.padBottom > 0 || dp.padTop > 0)) {
        var xDesc = dp.padLeft === dp.padRight ? ("±" + fmt(dp.padLeft) + " mm") : ("L: " + fmt(dp.padLeft) + " / R: " + fmt(dp.padRight) + " mm");
        var yDesc = dp.padBottom === dp.padTop ? ("±" + fmt(dp.padBottom) + " mm") : ("B: " + fmt(dp.padBottom) + " / T: " + fmt(dp.padTop) + " mm");
...
function rebuild(q) {
  ...
  var res = P.mode === "plate" ? buildPlate(P, seg) : buildBin(P, seg);
...
function stem() {
  if (P.mode === "plate") {
    ...
    if (dp.padLeft > 0 || dp.padRight > 0 || dp.padBottom > 0 || dp.padTop > 0) {
      var alignSuffix = (P.buf_x_align !== "center" ? "_" + P.buf_x_align : "") + (P.buf_y_align !== "center" ? "_" + P.buf_y_align : "");
...
["mode_bin", "mode_plate", "layout_grid", "layout_rows", "layout_cols", "plate_mode_units", "plate_mode_mm",
 "buf_x_left", "buf_x_center", "buf_x_right", "buf_y_bottom", "buf_y_center", "buf_y_top"].forEach(function (id) {
  var el = document.getElementById(id);
  if (el) el.addEventListener("change", function () { onChange(id.startsWith("mode") || id.startsWith("plate_mode") || id.startsWith("buf_")); });
});
...
function meshSig(p, seg) { return seg.corner + "|" + JSON.stringify(p); }
