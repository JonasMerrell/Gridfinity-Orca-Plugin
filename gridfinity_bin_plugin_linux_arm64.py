...
<div class="app">
  <aside class="panel">
    <header>
      <h1>Gridfinity Bin</h1>
      <p class="sub">Geometry matches <code>gridfinity_bin.scad</code></p>
    </header>
    ...
    <section>
      ...
      <pre id="cmd"></pre>
      <div class="btns"><button id="copyCmd">Copy OpenSCAD command</button></div>
    </section>
  </aside>
...
function derive(p) {
  ...
  var CW = (IW - (p.dx - 1) * p.wall) / p.dx;
  var CD = (ID - (p.dy - 1) * p.wall) / p.dy;
  var R_IN = Math.max(0, Math.min(R_TOP - p.wall, CW / 2 - 0.01, CD / 2 - 0.01));
  var depth = H_BODY - FLOOR;
  var fillet = Math.max(0, Math.min(p.fillet, CW / 2 - 0.01, CD / 2 - 0.01, depth));
  var scoopR = p.scoop ? Math.max(0, Math.min(p.scoopR, CD - 2 * fillet, depth)) : 0;
  var labelD = p.label ? Math.max(0, Math.min(p.labelD, CD - 1, depth)) : 0;
  return {
    OX: OX, OY: OY, H_BODY: H_BODY, FLOOR: FLOOR, IW: IW, ID: ID,
    CW: CW, CD: CD, R_IN: R_IN, depth: depth, fillet: fillet,
    scoopR: scoopR, labelD: labelD,
    TOP: H_BODY + (p.lip ? LIP_H : 0),
    support: Math.max(0, LIP_INSET - p.wall),
    pitchX: CW + p.wall, pitchY: CD + p.wall,
    valid: CW > 0.2 && CD > 0.2 && depth > 0.2
  };
}
...
function* binSteps(P, seg) {
  ...
  var comp = [], compSmall = [];
  var f = d.fillet;
  for (i = 0; i < P.dx; i++) for (j = 0; j < P.dy; j++) {
    var cx = (i - (P.dx - 1) / 2) * d.pitchX;
    var cy = (j - (P.dy - 1) / 2) * d.pitchY;
    comp.push(offsetLoop(rrLoop(d.CW, d.CD, d.R_IN, ni), cx, cy));
    compSmall.push(offsetLoop(
      rrLoop(d.CW - 2 * f, d.CD - 2 * f, Math.max(0, d.R_IN - f), ni), cx, cy));
  }
  ...
  for (var c = 0; c < comp.length; c++) {
    if (f > 1e-6) addBand(m, compSmall[c], d.FLOOR, comp[c], d.FLOOR + f, true);
    addBand(m, comp[c], d.FLOOR + f, comp[c], d.H_BODY, true);
  }
  addCap(m, OUT, f > 1e-6 ? compSmall : comp, d.FLOOR, false);
  ...
  for (i = 0; i < P.dx; i++) for (j = 0; j < P.dy; j++) {
    var ccx = (i - (P.dx - 1) / 2) * d.pitchX;
    var ccy = (j - (P.dy - 1) / 2) * d.pitchY;
    var x0 = ccx - d.CW / 2, x1 = ccx + d.CW / 2;

    if (d.scoopR > 1e-6) {
      var rs = d.scoopR, y0 = ccy - d.CD / 2, sec = [[y0, d.FLOOR]];
    ...
    if (d.labelD > 1e-6) {
      var ld = d.labelD, yb = ccy + d.CD / 2, tb = d.H_BODY;
...
function updateReadout() {
  ...
  document.getElementById("s_comp").textContent = fmt(d.CW) + " × " + fmt(d.CD) + " mm";
...
function updatePlateReadout() {
  ...
  document.getElementById("cmd").textContent =
    "Baseplate geometry is generated here, not by gridfinity_bin.scad.\n" +
    "Verified against GridFlock: volume within 0.01% of its OpenSCAD output.";
}
...
function stem() {
  ...
  return "gridfinity_" + P.gx + "x" + P.gy + "x" + P.gz +
         (P.dx * P.dy > 1 ? "_" + P.dx + "x" + P.dy : "");
}
...
document.getElementById("copyCmd").addEventListener("click", function (e) {
  var btn = e.target, original = btn.textContent, text = command();
  function done(msg) {
    btn.textContent = msg;
